...
func TestBuildBoardReport(t *testing.T) {
	...
	if report.Groups[1].Name != "No Sprint" {
		t.Fatalf("expected second group to be No Sprint, got %s", report.Groups[1].Name)
	}
	if len(report.Groups[0].Cards) != 3 {
		t.Fatalf("expected 3 current sprint cards, got %d", len(report.Groups[0].Cards))
	}
	if len(report.Groups[1].Cards) != 1 {
		t.Fatalf("expected 1 no sprint card, got %d", len(report.Groups[1].Cards))
	}
	if len(cards) != 4 {
		t.Fatalf("expected 4 flattened cards, got %d", len(cards))
	}

	wantIDs := []int{102, 101, 103, 104}
